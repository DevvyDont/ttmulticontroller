...
namespace TTMulti
{
    static class Win32
    {
        ...
        /// <summary>
        /// Gets the monitor handle at the given 0-based index (same order as EnumDisplayMonitors).
        /// Returns IntPtr.Zero if the index is out of range.
        /// </summary>
        internal static IntPtr GetMonitorHandleByIndex(int displayIndex)
        {
            ...
            EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, callback, IntPtr.Zero);
            if (displayIndex < 0 || displayIndex >= handles.Count)
                return IntPtr.Zero;
            return handles[displayIndex];
        }
        ...
        /// <summary>
        /// Gets the effective DPI of the monitor that contains the given window. Returns (96, 96) if the call fails.
        /// </summary>
        internal static (uint dpiX, uint dpiY) GetDpiForWindow(IntPtr hwnd)
        {
        ...
        /// <summary>
        /// Gets the effective DPI of the monitor at the given display index, or of the monitor containing the point (rx, ry) for Manual mode.
        /// Returns (96, 96) if the call fails.
        /// </summary>
        internal static (uint dpiX, uint dpiY) GetDpiForRegion(LayoutRegion region, int rx, int ry)
        {
            IntPtr hMonitor;
            if (region.Mode == LayoutRegionMode.Display && region.DisplayIndex >= 0)
            {
                hMonitor = GetMonitorHandleByIndex(region.DisplayIndex);
            }
            else
            {
                hMonitor = MonitorFromPoint(new Point(rx, ry), MONITOR_DEFAULTTONEAREST);
            }
            if (hMonitor == IntPtr.Zero)
                return (96, 96);
            if (GetDpiForMonitor(hMonitor, MDT_EFFECTIVE_DPI, out uint dpiX, out uint dpiY) != 0)
        ...
        /// <summary>
        /// Gets the effective DPI of the monitor that contains the given point (e.g. a window's top-left position).
        /// Use this to get the DPI of the monitor where a window will land when placed at (x, y).
        /// Returns (96, 96) if the call fails.
        /// </summary>
        internal static (uint dpiX, uint dpiY) GetDpiForPoint(int x, int y)
        {
